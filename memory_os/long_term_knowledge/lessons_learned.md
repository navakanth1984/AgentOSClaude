...
### 2026-07-12 — mvct-mrp-core Merge Train Consolidation & Port Listener Safety
*   **Key Concept:** Merged and consolidated 8 divergent PRs into master. Investigated and resolved a test suite execution hang.
*   **Outcome/Lessons:**
    *   **Node.js Event Loop Hangs**: Node.js test suites or script runners that import modules starting an Express server/socket listener in their top-level global scope will hang indefinitely. This is because the open port listener keeps the event loop active. Resolving this requires wrapping the server listener block with `if (require.main === module)`.
    *   **Concurrent Runtime Isolation**: Database-backed projection systems (such as the IOS integration tests) must use cleanly segregated class namespaces (`iosProjector.ts`, `iosCheckpointStore.ts`, etc.) to prevent collision with memory-based unit testing systems (M6B runtimes) when built and run concurrently in the same project directory.
*   **Actionable Takeaways:**
