...
source: "Repo: docs/superpowers/specs/2026-06-25-mvct-v1-design.md — merged via PR #2"
---

> [!info] Source of truth
> The canonical, version-controlled spec lives in the repo at
> `docs/superpowers/specs/2026-06-25-mvct-v1-design.md` and was merged to `master`
> via [PR #2](https://github.com/navakanth1984/AgentOSClaude/pull/2)
> (merge commit `44dd17f`). Implementation: `mvct-v1/` · Plan:
> `docs/superpowers/plans/2026-06-25-mvct-v1.md`. This vault copy is a readable
> mirror — edit the repo file, not this one, to avoid divergence.
>
> Related: [[2026-06-23-mvct-v1-blueprint]] · [[2026-06-23-transfer-detector-v0-brief]]
...
`GatePolicy{classification_required, confidence_floor, unlock_strategy}` with named `STRICT (0.90) / RESEARCH (0.65) / BINARY (0.0)`. Default = `STRICT`. The threshold is **not** an architectural law; it is a tunable the experiment sweeps.

### 3.3 Capability Auditor (`auditor.py`) — sole unlock authority
Calls `TransferSensor.read(transcript)`, applies the active `GatePolicy`; if satisfied, **mints a `PermissionToken`** (the only module permitted to). **Fail-closed:** any sensor/API failure → no token → answer stays locked.

### 3.4 PermissionToken (`permission.py`) — capability, not a boolean
HMAC-signed token with `issued_by, timestamp, classification, confidence, expires_at_turn, signature`. Enforces **Invariant 3**: only the Auditor can mint; everything else only *verifies*. `expires_at_turn` blocks replay. Mirrors capability-based OS design.

### 3.5 Socratic Interface (`socratic.py`) — model proposes, Python disposes
LLM (`gemini-2.5-flash`) generates the turn; while locked, a deterministic guard blocks any answer-leaking turn and the prompt forbids enumerating the canonical variables (which would also trip the detector's mimicry penalty). Unlocks only on a verified token.

### 3.6 KnowledgeProvider (`knowledge.py`) — abstraction kept, implementation trivial
`KnowledgeProvider` Protocol (`scenario_prompt`, `canonical_answer`, `canonical_independent_variables`, `leak_blocklist`); `HouseplantKnowledgeProvider` is static. Future graph/ontology/retrieval providers plug in without touching the Auditor or Interface.

### 3.7 Internalization Ledger (`ledger.py`) — pedagogical history
`sqlite3` (stdlib). Logs turns + computes **SAIR** (§6). Session ends on unlock (success) or quit (failure-state log).

### 3.8 Metrics Monitor (`monitor.py`) — runtime behavior (distinct from ledger)
Per gate evaluation: detector output, granted/denied, reason, latency, confidence, turn. Ledger = "what did the learner do?"; Monitor = "how did the gate behave?".

### 3.9 Runtime (`loop.py` interactive · `runner.py` scripted)
`loop.py` interactive demo; `runner.py` replays an annotated script through `control` and `experimental` and emits the comparison; `config.py` holds tunables + HMAC key source.

---

## 4. The Three Constitutional Invariants
Enforced in Python, never trusted to LLM prompt adherence (the CCA's `−∞` barrier reduced to MVP form).

- **Invariant 1 — No canonical answer tokens while locked.** Deterministic blocklist match. Fully enforceable.
- **Invariant 2 — No logically-equivalent complete solution while locked.** Best-effort via the canonical-variable blocklist; full semantic-equivalence detection is a known V1 gap (logged for manual audit).
- **Invariant 3 — Unlock can only originate from the Auditor.** Enforced structurally by `PermissionToken` HMAC minting.

---

## 5. Topic (`topics/houseplant.json`)
Reuses the detector's exact primary scenario (light, watering, drainage/soil, nutrients, pests/disease, temperature/humidity, root health) + the derived `leak_blocklist`. A code comment marks the domain **mandatory** (in-distribution constraint), not a default.

---

## 6. Metrics

- **SAIR — Self-Initiated Asset Invocation Rate** = self-initiated decomposition uses / opportunities. Target > 0.0 to prove baseline.
- **Unlock Latency** = `turn_transfer − turn_first_correct_reasoning` (script-annotated). Ideal 0; positive = hesitated; negative = unlocked early (safety failure).
- **Comparison output:** control vs. experimental on identical scripts — exposure timing, SAIR, divergence attributable to the gate.

---

## 7. Testing Strategy
TDD (enforcement before prompt). All tests run **offline** via the detector's dry-run mock (no API key, deterministic). Token forgery/expiry, guard leak-blocking, auditor mapping + fail-closed, SAIR math, unlock-latency, and the control-vs-experimental transition each have a test. **(28 passing.)**

---

## 8. Dependencies & Layout
Python stdlib (`sqlite3`, `hmac`, `json`, `dataclasses`) + `transfer-detector-v0` (path resolution) + `google-generativeai` (reused; single `GEMINI_API_KEY`, Tier 1/2). **No** React, FastAPI, NetworkX, or vector/graph DB. Package: `mvct-v1/` with `sensor / policy / permission / auditor / socratic / knowledge / ledger / monitor / loop / runner / config` + `topics/` + `scripts/` + `tests/`.

---

## 9. Deferred to Later Versions (YAGNI / anti-scope)
Knowledge Graph implementation · Causal Memory · Developmental Controller (moving attractor) · graded scaffold-withdrawal · multi-topic / multi-asset · dynamic topic generation · web UI · full semantic-equivalence leak detection · live-session unlock-latency · real-human detector ceiling re-validation.
...
Engineering risks are bugs we can close; research risks are open empirical questions the experiment exists to probe.

**Engineering risks (closable in V1):** API/LLM failure → fail-closed + injectable `llm_fn`; prompt leakage → guard (T1); token forgery/bypass → HMAC capability (T4); SQLite corruption → single-process, per-run DB files.

**Research risks (open, NOT closable by code):** detector construct validity (synthetic ceiling, §2.3); domain generalization (houseplant held fixed); confidence calibration (is 0.90 meaningful?); transfer-metric validity (does SAIR track internalization or a proxy?).

## 13. Success Hierarchy

Four independent levels; passing a lower level does **not** imply the next.
