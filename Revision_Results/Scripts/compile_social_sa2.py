...
def compile_social_sa2_results():
    metadata_dir = "social_sa2_extracted/metadata"
    files = sorted([f for f in os.listdir(metadata_dir) if f.endswith('.json')])
    
    full_text = "SOCIAL SA2 REVISION - FULL SET\n" + "="*30 + "\n\n"
    ...
    with open("Full_Social_SA2_Revision_Extract.txt", "w", encoding="utf-8") as f:
        f.write(full_text)
    print("Compiled full text to Full_Social_SA2_Revision_Extract.txt")

if __name__ == "__main__":
    compile_social_sa2_results()