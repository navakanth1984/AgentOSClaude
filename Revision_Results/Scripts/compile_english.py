...
def compile_english_results():
    metadata_dir = "english_extracted/metadata"
    files = sorted([f for f in os.listdir(metadata_dir) if f.endswith('.json')])
    
    full_text = "ENGLISH FA3 REVISION - FULL SET\n" + "="*30 + "\n\n"
    ...
    with open("Full_English_Revision_Extract.txt", "w", encoding="utf-8") as f:
        f.write(full_text)
    print("Compiled full text to Full_English_Revision_Extract.txt")

if __name__ == "__main__":
    compile_english_results()