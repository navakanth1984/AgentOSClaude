...
def compile_results():
    metadata_dir = "full_telugu_extracted/metadata"
    files = sorted([f for f in os.listdir(metadata_dir) if f.endswith('.json')])
    
    full_text = "TELUGU FA3 REVISION - FULL SET\n" + "="*30 + "\n\n"
    ...
    with open("Full_Telugu_Revision_Extract.txt", "w", encoding="utf-8") as f:
        f.write(full_text)
    print("Compiled full text to Full_Telugu_Revision_Extract.txt")
